class PostsController < ApplicationController
  ...
  def show
    ...
    @comment = Comment.new
    @event = @post.event
    @subscribed_activities = (current_user) ? EventActivitiesUser.find_all_by_user_id_and_event_id_and_status_id(current_user.id,@event.id,2) : nil
    @total_price = (current_user) ? @event.total_price(@subscribed_activities) : nil
  end
